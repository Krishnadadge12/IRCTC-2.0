...
function TrainDetails(){
  ...
  const handleSearchSubmit = (e) => {
    ...
    const filtered = dummyTrains.filter(train => {
      const matchFrom = !searchParams.from || train.from === searchParams.from;
      const matchTo = !searchParams.to || train.to === searchParams.to;
  ...
  // ✅ ONLY triggered when "View Details" button is clicked
  ...
  const timeMatches = (depTime, selectedTimes) => {
    ...
    const ranges = {
      morning: [4*60, 11*60 + 59],
      afternoon: [12*60, 17*60 + 59],
      night1: [18*60, 23*60 + 59],
      night2: [0, 3*60 + 59]
    };
  ...
  const applyExtraFilters = () => {
    ...
    if (extraFilters.classes.length > 0) {
      filtered = filtered.filter(train =>
        train.class.some(c => extraFilters.classes.includes(c))
      );
    }

    if (extraFilters.quotas.length > 0) {
      filtered = filtered.filter(train => extraFilters.quotas.includes(train.quota));
    }

    if (extraFilters.times.length > 0) {
      filtered = filtered.filter(train => timeMatches(train.departureTime, extraFilters.times));
    }
  ...
  const renderTrainList = () => {
    return (
      <div className="container mt-4 two-column-layout">
        {/* TOP SEARCH BAR */}
                  ...
                  <div key={train.id} className="train-card-vertical">
                    ...
                    <div className="train-card-footer">
                      <div className="train-details-mini">
                        <span>Classes: {train.class.join(", ")}</span>
                        <span>Quota: {train.quota}</span>
                      </div>
                      <div className="train-footer-right">
                        {/* ✅ ONLY THIS BUTTON TRIGGERS DETAILS */}
                        <button 
              ...
              <div className="filter-list">
                {uniqueClasses.map(cls => (
                  <label key={cls}>
                    <input
                      type="checkbox"
                      checked={extraFilters.classes.includes(cls)}
              ...
              <div className="filter-list">
                {uniqueQuotas.map(q => (
                  <label key={q}>
                    <input
                      type="checkbox"
                      checked={extraFilters.quotas.includes(q)}
                      onChange={() => toggleFilter('quotas', q)}
                    />
                    <span>{q}</span>
                  </label>
                ))}
    ...
    return (
      <div className="container mt-4">
        ...
        <div className="card" style={{ marginTop: "20px" }}>
          ...
          <button className="btn-book" onClick={() => alert("Booking functionality coming soon!")}>Book Now</button>
        </div>
      </div>
    );
  };
