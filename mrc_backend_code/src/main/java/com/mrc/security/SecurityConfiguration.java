...
public class SecurityConfiguration {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
		...
		request.requestMatchers("/swagger-ui/**","/v3/api-docs/**",
				"/users/signin","/patients/signup","/doctors","/users/pwd-encryption").permitAll()
		//ONLY for browser app - React app - permit all in flight requests
		.requestMatchers(HttpMethod.OPTIONS).permitAll()
		//protected endpoints - only for user role - patient
		.requestMatchers(HttpMethod.POST, "/appointments/book")
		 .hasRole("PATIENT")
		//protected endpoints - only for user role - doctor | admin
		 .requestMatchers("/appointments/doctors/{userId}/upcoming",
				 "/doctors/{userId}").hasAnyRole("DOCTOR","ADMIN")
		//protected endpoints - only for user role - admin
		 .requestMatchers(HttpMethod.GET,"/patients").
		 hasRole("ADMIN")		
		//3. Protect all remaining end points (apply authentication lock)
